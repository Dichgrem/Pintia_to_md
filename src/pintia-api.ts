...
export class PintiaAPI {
	...
	public async getMyProblemSets(activeOnly: boolean = true) {
		return makePintiaRequest<ProblemSetsResponse>(
			"/problem-sets",
			this.sessionId,
			{
				params: activeOnly
	...
	public async createProblemSetExams(problem_set_id: string) {
		...
		return await makePintiaRequest<ExamResponse>(
			`/problem-sets/${problem_set_id}/exams`,
			this.sessionId,
			{ method: "POST" },
		);
	}

	public async getProblemSetExams(
		...
		return makePintiaRequest<ExamResponse>(
			`/problem-sets/${problem_set_id}/exams`,
			this.sessionId,
		);
	}
	...
	public async getProblemSetSummaries(problem_set_id: string) {
		return makePintiaRequest<ProblemSummariesResponse>(
			`/problem-sets/${problem_set_id}/problem-summaries`,
			this.sessionId,
		);
	}

	public async getExamProblems(
		exams_id: string,
		problem_set_id: string,
		problem_type: string,
	) {
		return makePintiaRequest<ExamProblemsResponse>(
			`/problem-sets/${problem_set_id}/exam-problems`,
			this.sessionId,
			{
	...
	public async getProblemStatus(exams_id: string, problem_set_id: string) {
		return makePintiaRequest<ProblemStatusResponse>(
			`/exams/${exams_id}/problem-sets/${problem_set_id}/problem-status`,
			this.sessionId,
		);
	}

	public async getProblemSubmission(
		exams_id: string,
		problem_set_id: string,
		problem_id: string,
	) {
		return makePintiaRequest<any>(
			`/exams/${exams_id}/problem-sets/${problem_set_id}/exam-problem-submissions/${problem_id}`,
			this.sessionId,
		);
	}
	...
	public async getExamSubmissions(exams_id: string, problem_set_id: string) {
		return makePintiaRequest<any>(
			`/exams/${exams_id}/problem-sets/${problem_set_id}/submissions`,
			this.sessionId,
		);
	}
...
async function makePintiaRequest<T>(
	...
	if (!response.ok) {
		throw new Error(
			`Faild to fetch Pintia API, http code ${response.status}, check if environment variable PINTIA_SESSION_ID is set`,
		);
	}
